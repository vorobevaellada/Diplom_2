...
class TestOrderCreation:
    ...
    def test_order_creation_authenticated_user(self, create_user):
        ...
        with allure.step("Создание заказа"):
            order_creation = requests.post(Urls.ORDER_CREATION,
                                        data=data,
                                        headers={'Authorization': access_token})
        assert (order_creation.status_code == 200 and 
                order_creation.json().get("order"))
    ...
    def test_create_without_ingredient(self):
        ...
        with allure.step("Создание заказа"):
            order_creation = requests.post(Urls.ORDER_CREATION, data=data)
        assert (400 == order_creation.status_code and
                order_creation.json().get('message') == ResponseMessages.missing_ingredient)